...
def test_full_pipeline_orders_conditioning_before_derived_without_duplicate_depressions(
    ...
    assert observed.index("phase1_terrain_conditioning") < observed.index("phase1_terrain_derived")
...
def test_invariant_1_all_layers_aligned():
    ...
    for path in buffered_paths:
        with rasterio.open(path) as ds:
            buffered_grid.assert_aligned(ds)
...
def test_invariant_d4_pit_count_zero_on_conditioned_dem():
    ...
    assert n_outside == 0, f"D4 pit count outside retained basins is {n_outside:,}, expected 0."
