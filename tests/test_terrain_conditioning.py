...
Tests the mathematical invariants of depression-aware conditioning:
  ...
  Invariant C:  Bound on repair magnitude: max(cut) <= cut_max (2.60 m) and max(fill) <= fill_max (3.25 m).
  Invariant R:  Retained basin mask invariance: exactly 0 cells modified inside retained basins (classes 1..4).
...
def test_invariant_a1_d4_pits_subset_of_retained_union_residual():
    """All D4 pits in conditioned DEM must reside inside R union RESIDUAL (basin_class > 0).

    Pits outside (R union RESIDUAL) must be exactly 0.
    """
...
def test_invariant_a2_residual_pits_bounded_and_enumerated():
    """Residual repair failures must agree across all artifacts, be bounded, and be enumerated.

    V8 Seam & Cross-Artifact Assertion:
    The producer writes its guaranteed properties into the manifest, and the consumer asserts
    exact consistency across the three residual representations:
      1. basin_residuals.csv (row count == registered residual objects in buffered domain)
      2. basin_class_buffered.tif (class 5 count == residual cells in buffered domain)
      3. basin_class.tif (class 5 count == residual cells in canonical unbuffered domain)
      4. retained_basins.csv (class == 'residual' row count)
      5. terrain_conditioning manifest.json (declared buffered & canonical counts)

    Bound rule: Residual pits are unresolved D4 local minima on protected infrastructure
    (conveyance channels, buildings). Under honest domain-density scaling, residual cells must not
    exceed 0.75% of domain cells (95,463 buffered cells; 90,186 canonical cells).
    """
    ...
    assert int((buf_cutout == 5).sum()) == n_residuals_canon_realized, (
        "Invariant A2 failed: Canonical slice of basin_class_buffered.tif disagrees with basin_class.tif"
    )
    ...
    for col in ["id", "outlet_cell", "max_depth_m", "failure_reason"]:
        assert col in df_residuals.columns, f"Invariant A2 failed: Missing column {col} in residuals register"
...
def test_invariant_c_cut_and_fill_bounds():
    ...
    misclassification detector. The shallowest retained basin is 0.73 mm deep, so no
    scalar cap can both permit legitimate slope repairs (up to 2.55 m) and prevent
    basin drainage.

    Retained basin geometry and volume protection is strictly enforced by Invariant R
    (`test_invariant_retained_basin_interior_unmodified`), which replaced Invariant C's
    former classification-detector role.
    """
    ...
    # Fills on road and waterway conveyance channels must be 0
    full_diff = post - pre
...
def test_invariant_retained_basin_interior_unmodified():
    ...
    Every cell belonging to candidate retained basins (classes 1..4: Storage, Quarry,
    Landfill, Uncertain) must have elevation identically preserved between pre-breach
    and post-breach DEMs (|post - pre| <= 1e-4 m).
    """
    pre_path = INTERIM / "dem_conditioned_prebreach.tif"
    post_path = INTERIM / "dem_conditioned_postbreach.tif"
    class_buf_path = INTERIM / "basin_class_buffered.tif"
    ...
    # Retained candidate basins: classes 1 (Storage), 2 (Quarry), 3 (Landfill), 4 (Uncertain)
    retained_mask = (basin_class >= 1) & (basin_class <= 4) & valid
    n_retained_cells = int(retained_mask.sum())
    assert n_retained_cells > 0, "Invariant R failed: No retained basin cells found in class raster."
...
def test_invariant_retained_basin_mask_mutation_fails_red():
    ...
    road_mask_path = INTERIM / "road_segment_id_buffered.tif"
    waterway_mask_path = INTERIM / "distance_to_drain_buffered.tif"
    _skip_unless_exist(pre_path, class_buf_path, building_mask_path, road_mask_path, waterway_mask_path)
    ...
    # Extract a 500x500 tile containing lake basins
    ...
    assert n_modified_mut > 0, "Mutation check failed: Omitting retained_basin_mask should have modified basin cells."
