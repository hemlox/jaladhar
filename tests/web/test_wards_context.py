"""Ward/locality context assets (WF-6 U0f): parse, reproject, join, index, emit.

Verification scope is stated beside every check (V7).  The audit figures this
suite reconciles against were measured independently of this module's code
(2026-08 audit of the September 2022 replay product) and are the baseline; the
module's own outputs are the candidate.  No assertion below compares a
candidate to a baseline this repo produced (V3).

Red-under-mutation demonstrations (V5) live in
``TestRedUnderMutation`` and name their deliberate mutations in docstrings.
"""
...
# Audit baselines (independent of this module).
...
AUDIT_CLASSIFIED_NAMED_IMPLIED = 10318  # implied by 9,404 / 0.9114
AUDIT_CENTROID_WITHIN = 9404  # module realises 9402; delta recorded, not tuned away
...
class TestParsingAndReprojection:
    def test_placemark_counts_match_audit(self) -> None:
        """Scope: every Placemark element in both realised KML files.

        If parsing dropped or duplicated a placemark, the counts would move off
        the audit's independent 243/225.
        """
    ...
    def test_reprojection_area_band(self, wards_by_year: dict[int, object]) -> None:
        """Scope: whole-city total area per vintage after WGS84->EPSG:32643.

        A reprojection unit error (degrees vs metres, wrong zone) moves the
        total area by orders of magnitude; the band [700, 730] km2 pins it to
        the realised BBMP extent (audit-quoted ~716.9 km2).
        """
        ...
        for year, frame in wards_by_year.items():
            ...
            # EPSG:32643 Bengaluru window: easting ~760-810 km, northing ~1.41-1.47 Mm
...
class TestDivergenceReconciliation:
    def test_name_split_matches_audit(self) -> None:
        """Scope: full ward-name sets of both vintages under the reconciled
        match rule (whitespace-collapsed lowercase, trailing 'ward' stripped).

        Plain lowercase gives 86/68/157 -- the rule itself is part of what is
        under test; if the rule regressed the split would not reconcile.
        """
    ...
    def test_area_delta_reconciles_audit(self) -> None:
        """Scope: common-named ward pairs across both vintages.

        The median is reported under BOTH pairing rules with derivations named;
        the audit's quoted figure must reconcile under at least one, and the
        worst ward (by |dA|/min) must be mallasandra at ~183.6%.
        """
...
class TestSegmentWardJoin:
    def test_denominator_reconciles_audit_implied(self, join_result: dict) -> None:
        """Scope: all classified named segments (10 highway classes x non-null
        name) in the realised roads_centrelines.gpkg.

        The audit quoted 91.14% joined; 9,404 / 0.9114 implies exactly this
        denominator, so reproducing it confirms the subset definition.
        """
    ...
    def test_centroid_within_count_beside_audit(self, join_result: dict) -> None:
        """Scope: centroid-within assignments against the 2022 canonical layer,
        CRS EPSG:32643, centroid = shapely line centroid.

        This module realises 9,402 (91.12%); the audit's independent figure was
        9,404 (91.14%).  The two-segment residual is recorded here and in the
        manifest rather than tuned away; the rate guard below keeps the claim
        honest while the exact count pins realised behaviour.
        """
    ...
    def test_assignments_independently_recomputed(self, join_result: dict) -> None:
        """Scope: deterministic sample (every 37th assigned segment, n=254 of
        9,402) verified through a DIFFERENT mechanism than production: direct
        shapely point.within(polygon) over all 243 ward polygons instead of
        geopandas sjoin's STRtree path.

        If sjoin mis-assigned boundary segments, this linear-scan recomputation
        would disagree.
        """
    ...
    def test_point_frame_preserves_rows(self) -> None:
        """Production guard: the point frame built for the sjoin must carry
        every source row.  Motivation: under this environment's pandas 3.0.5,
        ``GeoDataFrame(dict, geometry=<GeoSeries>)`` silently returned 1,247 of
        10,318 rows during U0f scratch verification -- production therefore
        passes an ndarray and asserts the row count (see _safe_points_frame).
        If that guard regressed to the trap construction, this test reddens.
        """
...
class TestEmittedAssetsRealizedState:
    """Runs the REAL CLI from clean state into a temp context dir (V4/V12):
    identical inputs, isolated outputs, then asserts the bytes on disk."""
    ...
    def test_bin_layout_decodes_against_meta(self, build_dir: Path) -> None:
        """Scope: both emitted vintage bundles, full byte layout.

        Independent decode via struct (not the module's np.frombuffer path):
        header count, offset span, exact byte length, finite coordinates.
        """
    ...
    def test_manifest_lifecycle_completed(self, build_dir: Path) -> None:
        """Rule 6: manifest carries run-start fields AND terminal state, and
        output SHAs on disk actually hash to the recorded values (V1)."""
    ...
    def test_no_pin_codes_anywhere(self, build_dir: Path) -> None:
        """No PIN codes are held (rule 3); a six-digit-only token anywhere in
        the index would mean one was sought or synthesised."""
...
class TestRedUnderMutation:
    """V5: each test applies a deliberate mutation and requires the validator
    to redden.  Mutations are named in the docstrings."""

    def test_truncated_coordinate_stream_detected(self, clean_bundle: tuple) -> None:
        """MUTATION: drop the last coordinate pair (8 bytes) from the realised
        bundle.  validate_bundle must fail on vertex-count/offset mismatch --
        this is the 'corrupt a coordinate count -> red' demonstration."""
    ...
    def test_flipped_vertex_declaration_detected(self, clean_bundle: tuple) -> None:
        """MUTATION: inflate metadata n_vertices by +64 without touching the
        blob.  The declared-vs-realised mismatch must redden."""
    ...
    def test_ring_range_bookkeeping_mutation_detected(self, clean_bundle: tuple) -> None:
        """MUTATION: replace one ward's vertex ring_ranges with ring indices
        ([i, i+1]) -- the exact bookkeeping slip this module shipped once and
        the strengthened validator now catches."""
    ...
    def test_manifest_written_at_run_start_on_failure(
        ...
        """MUTATION: corrupt the 2023 KML source mid-build.  Rule 6 requires
        that even a failing run leaves a manifest written BEFORE the heavy
        work: status failed, but git_sha and the 2022 input SHA already
        recorded.  If provenance only existed when nothing went wrong, the
    ...
    def test_missing_context_config_aggregates_every_key(
        ...
        """MUTATION (V5): configs/context.yaml removed entirely.

        Expected observation: startup resolution raises WardContextConfigError
        whose message names the missing file AND every required key together
        (rule 7) -- not a KeyError on the first downstream access after the
        heavy work. Green state: TestContextConfigSurface reads the real file.
        """
    ...
    def test_corrupt_context_yaml_lists_all_unresolved_keys(
        ...
        """MUTATION (V5): configs/context.yaml corrupted to a bare scalar
        (parses as YAML but not into the required mapping).

        Expected observation: ONE aggregated error reporting the shape problem
        and listing ALL required keys as unresolved. The same mutation against
        the old code path would have surfaced as a KeyError mid-build instead.
        """
    ...
    def test_cli_refuses_to_start_on_missing_config(
        ...
        """MUTATION (V5): configs/context.yaml absent at CLI level.

        Rule 7 fail-fast is realized here: build() resolves the config as its
        FIRST statement, so the run dies before the compute-budget banner or
        any heavy work -- observed by the absence of that banner in output.
        """
        ...
        # Fail fast: resolution happens before ANY work output is produced.
...
class TestContextConfigSurface:
    """configs/context.yaml is the single source of truth for ward vintage
    (owner adjudication item 4): resolution, manifest recording, CLI override.

    Tests that repoint CONTEXT_CONFIG_PATH must reset the module's config
    cache -- a cached payload would be a declaration of intent masquerading
    as realized state.
    """

    def test_config_file_resolution_honored(self) -> None:
        """Scope: the realised configs/context.yaml at repo root, full parse.

        V2 observable: values come from the file on disk; if someone edited
        the yaml or reintroduced a code constant, this read-back would move.
        """
        wards_mod.reset_context_config_cache()
    ...
    def test_no_code_fallback_constants_remain(self) -> None:
        """The relocated code constants must be GONE (owner direction): any
        surviving module attribute would be a shadow source of truth."""
    ...
    def test_manifest_records_config_surface(self, build_dir: Path) -> None:
        """Scope: the completed build's manifest vintage_decision block --
        config_source, rule string VERBATIM, condensed owner rationale."""
        manifest = json.loads((build_dir / "context_build_manifest.json").read_text("utf-8"))
        ...
        # Condensed owner quote: numbers/boundaries changed substantially in
        # the 2023 restructure; names comparatively stable.
    ...
    def test_cli_override_wins_over_config(self, tmp_path_factory: pytest.TempPathFactory) -> None:
        ...
        # Precedence is realized in the OUTPUTS, not just recorded intent:
        # the join table and search index are built against 2023.
