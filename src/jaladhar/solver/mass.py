    ...
    def accumulate(
        ...
        with torch.no_grad():
            ...
            if step_drained_m < -tol * n_cells:
                raise RuntimeError(
                    f"Negative total drainage this step: {step_drained_m:.6e} m (tol {-tol * n_cells:.3e} m)"  # noqa: E501
                )
            if step_infil_m < -tol * n_cells:
                raise RuntimeError(
                    f"Negative total infiltration this step: {step_infil_m:.6e} m (tol {-tol * n_cells:.3e} m)"  # noqa: E501
                )
            ...
            if min_step_infil < -tol:
                raise RuntimeError(
                    f"Negative cell infiltration detected: {min_step_infil:.6e} m (tol {-tol:.3e} m)"  # noqa: E501
                )

            self.v_current = self.volume(h)
            if isinstance(rain_rate_m_s, torch.Tensor):
                self.rain_in += (
                    float(rain_rate_m_s.sum(dtype=torch.float64)) * dt * self.cell_area_m2
                )
            else:
            ...
            if self.drain_out < -tol * n_cells * self.cell_area_m2:
                raise RuntimeError(
                    f"Cumulative drainage negative: {self.drain_out:.6e} m^3 (tol {-tol * n_cells * self.cell_area_m2:.3e} m^3)"  # noqa: E501
                )
            if self.infil_out < -tol * n_cells * self.cell_area_m2:
                raise RuntimeError(
                    f"Cumulative infiltration negative: {self.infil_out:.6e} m^3 (tol {-tol * n_cells * self.cell_area_m2:.3e} m^3)"  # noqa: E501
                )
