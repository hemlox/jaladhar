...
> **How to use this file:** Drop it in the repo root as `SPEC.md` (and optionally symlink/copy the "Standing rules" section into `CLAUDE.md`). Start Claude Code and say: *"Read SPEC.md. Execute Phase 0. Stop at the gate and report."* Work phase by phase. Do not let it run ahead — the gates exist because a wrong answer early makes everything downstream worthless.
