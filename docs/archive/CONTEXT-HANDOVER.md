...
### Latest commits
- `b79cf2d` — terrain D4 + buffer rebuild
- `b79cf2d` — run.py config fix, `_resolve_config`, `--smoke`, manifest lifecycle. **This is the acceptance-run commit.**

> Run manifests in `runs/` record the commit SHA that was current when each run executed.
> `runs/` is gitignored, so those values were never repointed as the tree moved. Match a
> manifest to its run by stage name and timestamp, not by hash.
...
Acting on the old instruction produced `docs/PROMPT.md` as a byte-identical second copy in commit `b79cf2d`. **Root `PROMPT.md` is the master and is what every prompt and agent in this repo's history references.** The duplicate has been deleted.
...
2. `git log --oneline -10` and `git status` — confirm you're at `b79cf2d` or later and the tree is clean. **If the tree is dirty, stop and report it before anything else** — three tracked files were once deleted unstaged and an untracked one was lost outright, which is why `docs/d4-audit.md` and `docs/walkthrough.md` are marked as reconstructions.
...
See [`phase3-writeup.md`](../phases/phase3-writeup.md) — the gate ran, its extent verdict was withdrawn after an independent audit, and the forcing under-read is the live finding. Raw agent reports and the audit are under [`reports/`](reports/).
